...
import type { CommunicationIngressProvider, ConnectionState, ExternalIdentity, InboundCommunication, ScheduleIngressProvider, ScheduleRecord, SourceAccount } from './contracts.js';
...
const accountFromRow = (row: any): SourceAccount => ({ id: row.id, provider: row.provider, capability: row.capability, displayName: row.display_name, accountIdentifier: row.account_identifier, enabled: row.enabled, connectionState: row.connection_state, cursorState: row.cursor_state ?? {}, lastAttemptAt: row.last_attempt_at?.toISOString(), lastSuccessfulSyncAt: row.last_successful_sync_at?.toISOString(), lastErrorCode: row.last_error_code ?? undefined });
...
export class IngressService {
  ...
  async createSourceAccount(input: Omit<SourceAccount, 'id' | 'connectionState' | 'cursorState'> & { connectionState?: ConnectionState; cursorState?: Record<string, unknown> }) {
    const id = randomUUID();
    await this.db.query('insert into director_source_accounts(id,provider,capability,display_name,account_identifier,enabled,connection_state,cursor_state) values($1,$2,$3,$4,$5,$6,$7,$8)', [id, input.provider, input.capability, input.displayName, input.accountIdentifier, input.enabled, input.connectionState ?? 'UNAVAILABLE', json(input.cursorState ?? {})]);
