...
export interface ChiefOfStaffService {
  ...
  recordDecision(id: string, selectedOption: string): Promise<void>;
}
...
export class DirectorApiService implements ChiefOfStaffService {
  ...
  async getToday(): Promise<TodayState> { const [x,waitingFor]=await Promise.all([this.get('/today'),this.getOpenLoops()]); return { needsYou:x.needsYou.map((i:any)=>({id:i.id,title:i.title,subtitle:i.rationale.join(' • '),deadline:i.deadline?new Date(i.deadline).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}):undefined,status:'needs_you',source:{system:'CHIEF_OF_STAFF',referenceId:i.trace?.sourceRecordId},personId:i.person?.external_id==='anna-meyer'?'u1':undefined,projectId:i.project?.external_id==='harbour'?'p1':undefined,type:i.classification==='DECISION_REQUIRED'?'decision':'review',context:{why:i.rationale,details:i.trace?.interpretationCandidateId?'Validated interpretation with source evidence.':undefined}})),calendar:(x.calendar??[]).map((event:any)=>({id:event.id,time:event.all_day?'All day':new Date(event.starts_at).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}),title:event.title,location:event.location??'',type:'meeting'})),waitingFor,handled:(x.handled??[]).filter((item:any)=>item.artifactId).map((item:any)=>({id:item.id,description:item.description,time:new Date(item.completedAt).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}),agentId:item.agentId,artifactId:item.artifactId,externalActions:item.externalActions})),insights:[],timeline:[] }; }
  async getAttention(){ const x=await this.get('/attention'); return x.items.map((i:any)=>({id:i.id,title:i.title,subtitle:i.rationale.join(' • '),deadline:i.deadline?new Date(i.deadline).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}):undefined,status:'needs_you',source:{system:'CHIEF_OF_STAFF',referenceId:i.trace?.sourceRecordId},personId:i.person?.external_id==='anna-meyer'?'u1':undefined,projectId:i.project?.external_id==='harbour'?'p1':undefined,type:i.classification==='DECISION_REQUIRED'?'decision':'review',context:{why:i.rationale,details:i.trace?.interpretationCandidateId?'Validated interpretation with source evidence.':undefined}})); }
  ...
  async recordDecision(id:string, selectedOption:string){ const r=await fetch(`${this.baseUrl}/decisions/${id}/record`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({selectedOption})});if(!r.ok)throw new Error(`Decision failed: ${r.status}`); }
}
