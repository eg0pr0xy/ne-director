...
import { agentControlApi, agentControlApiEnabled, type AgentProjection, type Permission } from '../services/agentControls';
...
const AgentsSettingsSection = () => {
  ...
  const refresh = async () => { if (!agentControlApiEnabled) return; setLoading(true); try { const values = await agentControlApi.agents(); setAgents(values); setError(null); if (selected) setSelected(values.find(value => value.charter.agentId === selected.charter.agentId) ?? null); } catch { setError('Agent authority is unavailable. No mock policy is shown.'); setAgents([]); setSelected(null); } finally { setLoading(false); } };
  useEffect(() => { refresh(); }, []);
