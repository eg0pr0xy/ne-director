...
import { agentControlApi, agentControlApiEnabled, type AgentProjection, type AgentRuntimeHealth, type Permission } from '../services/agentControls';
...
const CATEGORIES = [
  ...
  { id: 'agents', label: 'Agents', section: 'CHIEF OF STAFF' },
...
export const SettingsPage = () => {
  const { settings, authorityAvailable, updateSettings, resetSettings, projects, people, settingsCategory, setSettingsCategory } = useApp();
...
const SettingsContent = ({ activeCategory, settings, authorityAvailable, updateSettings, projects, people }: any) => {
  ...
  if (agentControlApiEnabled && !authorityAvailable && (activeCategory === 'behavior' || activeCategory === 'autonomy')) return <AuthorityUnavailable />;
...
const AuthorityUnavailable = () => <div className="border border-[#E56A54]/30 bg-[#E56A54]/10 rounded-2xl p-6 text-text-primary"><h2 className="text-xl font-medium mb-2">Operator controls unavailable</h2><p className="text-sm text-text-secondary">Authority-bearing settings require the Director API and PostgreSQL. No browser-stored policy is shown or applied.</p></div>;

const permissionLabel = (value: string) => value === 'SUGGEST_ONLY' ? 'Suggest Only' : value === 'APPROVAL_REQUIRED' ? 'Approval Required' : 'Allowed';
const permissionRank = (value: Permission) => value === 'SUGGEST_ONLY' ? 0 : value === 'APPROVAL_REQUIRED' ? 1 : 2;
const agentStateClass = (state: string) => state === 'ACTIVE' ? 'bg-green-500/10 text-green-500' : state === 'QUALITY_BLOCKED' || state === 'CAPABILITY_LIMITED' || state === 'NOT_CONNECTED' ? 'bg-amber-500/10 text-amber-600' : 'bg-border text-text-secondary';

const AgentsSettingsSection = () => {
  const [agents, setAgents] = useState<AgentProjection[]>([]);
  const [runtime, setRuntime] = useState<AgentRuntimeHealth | null>(null);
  const [selected, setSelected] = useState<AgentProjection | null>(null);
  const [loading, setLoading] = useState(agentControlApiEnabled);
  const [error, setError] = useState<string | null>(null);
  const refresh = async () => { if (!agentControlApiEnabled) return; setLoading(true); try { const [values, runtimeHealth] = await Promise.all([agentControlApi.agents(), agentControlApi.runtimeHealth()]); setAgents(values); setRuntime(runtimeHealth); setError(null); if (selected) setSelected(values.find(value => value.charter.agentId === selected.charter.agentId) ?? null); } catch { setError('Agent authority is unavailable. No mock policy is shown.'); setAgents([]); setRuntime(null); setSelected(null); } finally { setLoading(false); } };
  useEffect(() => { refresh(); }, []);
  if (!agentControlApiEnabled) return <AuthorityUnavailable />;
  if (selected) return <AgentDetail agent={selected} onBack={() => setSelected(null)} onUpdated={async (agent) => { setSelected(agent); await refresh(); }} />;
  return <div className="space-y-8 animate-in fade-in duration-300"><header><h2 className="text-xl font-medium text-text-primary mb-2">AGENTS</h2><p className="text-text-secondary">Your Chief of Staff coordinates a specialist team. You work with one Director; specialists operate behind the scenes.</p></header>{loading && <p className="text-sm text-text-muted">Loading canonical operator policy…</p>}{error && <div className="border border-[#E56A54]/30 bg-[#E56A54]/10 rounded-xl p-4 text-sm text-text-primary">{error}</div>}{!loading && !error && <><section className="bg-surface border border-border rounded-2xl p-5"><div className="flex justify-between gap-4"><div><h3 className="text-sm font-medium text-text-primary">Runtime</h3><p className="text-xs text-text-secondary mt-1">Internal read and preparation work only. External actions remain unavailable.</p></div><span className={cn('text-[10px] px-2 py-0.5 rounded uppercase tracking-wider h-fit', agentStateClass(runtime?.state ?? 'DISABLED'))}>{runtime?.state ?? 'UNAVAILABLE'}</span></div>{runtime && <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-xs mt-4"><div><span className="text-text-muted block">Completed today</span>{runtime.completed_today}</div><div><span className="text-text-muted block">Queued</span>{runtime.queued}</div><div><span className="text-text-muted block">Blocked</span>{runtime.blocked_policy + runtime.blocked_capability}</div><div><span className="text-text-muted block">Last completed</span>{runtime.last_completed_at ? new Date(runtime.last_completed_at).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}) : '—'}</div><div><span className="text-text-muted block">Last tick</span>{runtime.lastTickAt ? new Date(runtime.lastTickAt).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}) : '—'}</div></div>}</section><AgentOverviewCard agent={agents.find(agent => agent.charter.agentId === 'CHIEF_OF_STAFF')} onConfigure={setSelected} prominent /><div><h3 className="text-xs font-bold tracking-widest text-text-muted mb-3">SPECIALISTS</h3><div className="space-y-3">{agents.filter(agent => agent.charter.agentId !== 'CHIEF_OF_STAFF').map(agent => <div key={agent.charter.agentId}><AgentOverviewCard agent={agent} onConfigure={setSelected} /></div>)}</div></div></>}</div>;
};

const AgentOverviewCard = ({ agent, onConfigure, prominent = false }: { agent?: AgentProjection; onConfigure: (agent: AgentProjection) => void; prominent?: boolean }) => {
  if (!agent) return null;
  return <div className={cn('bg-surface border border-border rounded-2xl p-5', prominent && 'border-border-focus')}><div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2 mb-1"><h3 className="font-medium text-text-primary">{agent.charter.displayName}</h3><span className={cn('text-[10px] px-2 py-0.5 rounded uppercase tracking-wider', agentStateClass(agent.derivedState))}>{agent.derivedState.replaceAll('_', ' ')}</span></div><p className="text-sm text-text-secondary">{agent.charter.shortDescription}</p></div><button className="px-3 py-1.5 text-xs border border-border rounded-lg text-text-primary hover:bg-border" onClick={() => onConfigure(agent)}>Configure</button></div><div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3 text-xs"><div><span className="text-text-muted block">Policy</span>{agent.operatorPolicy.status}</div><div><span className="text-text-muted block">Delegation</span>{agent.operatorPolicy.delegationMode === 'MANUAL_ONLY' ? 'Manual only' : 'Automatic when allowed'}</div><div><span className="text-text-muted block">Capabilities</span>{agent.effectiveCapabilitySummary.available} available / {agent.effectiveCapabilitySummary.limited} limited</div><div><span className="text-text-muted block">Model strategy</span>{agent.operatorPolicy.modelStrategy.replaceAll('_', ' ')}</div></div></div>;
};

const AgentDetail = ({ agent, onBack, onUpdated }: { agent: AgentProjection; onBack: () => void; onUpdated: (agent: AgentProjection) => Promise<void> }) => {
  const [saving, setSaving] = useState(false); const [error, setError] = useState<string | null>(null);
  const patch = async (value: Omit<Parameters<typeof agentControlApi.updatePolicy>[1], 'version'>) => { setSaving(true); try { const updated = await agentControlApi.updatePolicy(agent.charter.agentId, { version: agent.operatorPolicy.version, ...value }); setError(null); await onUpdated(updated); } catch (reason) { setError(reason instanceof Error ? reason.message.replaceAll('_', ' ') : 'Policy could not be saved.'); } finally { setSaving(false); } };
  return <div className="space-y-8 animate-in fade-in duration-300"><button onClick={onBack} className="text-sm text-text-secondary hover:text-text-primary">← Agents</button><header><div className="flex items-center gap-3"><h2 className="text-xl font-medium text-text-primary">{agent.charter.displayName}</h2><span className={cn('text-[10px] px-2 py-0.5 rounded uppercase tracking-wider', agentStateClass(agent.derivedState))}>{agent.derivedState.replaceAll('_', ' ')}</span></div><p className="text-text-secondary mt-2">{agent.charter.purpose}</p></header>{error && <div className="border border-[#E56A54]/30 bg-[#E56A54]/10 rounded-xl p-3 text-sm text-text-primary">{error}</div>}<section className="border-t border-border pt-5"><h3 className="text-xs font-bold tracking-widest text-text-muted mb-3">STATUS</h3>{agent.charter.cannotDisable ? <p className="text-sm text-text-secondary">Chief of Staff stays active as the primary experience. Global Pause All may pause the team.</p> : <button disabled={saving} onClick={() => patch({ status: agent.operatorPolicy.status === 'ACTIVE' ? 'PAUSED' : 'ACTIVE' })} className="px-4 py-2 rounded-lg text-sm border border-border text-text-primary hover:bg-border">{agent.operatorPolicy.status === 'ACTIVE' ? 'Pause agent' : 'Resume agent'}</button>}</section><section className="border-t border-border pt-5"><h3 className="text-xs font-bold tracking-widest text-text-muted mb-3">RESPONSIBILITIES</h3><ul className="space-y-1 text-sm text-text-secondary">{agent.charter.responsibilities.map(item => <li key={item}>• {item}</li>)}</ul></section><section className="border-t border-border pt-5"><h3 className="text-xs font-bold tracking-widest text-text-muted mb-3">AUTONOMY</h3><div className="space-y-3">{agent.capabilities.filter(capability => capability.globalAutonomyKey).map(capability => <div key={capability.capabilityId} className="flex flex-col md:flex-row md:items-center justify-between gap-3 bg-surface p-3 rounded-lg"><div><div className="text-sm text-text-primary">{capability.capabilityId.replaceAll('_', ' ')}</div><div className="text-xs text-text-muted">Effective: {permissionLabel(capability.effectiveAutonomy)} · {capability.effectiveState.replaceAll('_', ' ')}</div></div><select disabled={saving} value={capability.operatorOverride ?? ''} onChange={event => patch({ capabilityOverrides: { [capability.capabilityId]: event.target.value ? event.target.value as Permission : null } })} className="bg-bg border border-border text-text-primary text-xs rounded px-2 py-1.5"><option value="">Inherit global ({permissionLabel(capability.effectiveAutonomy)})</option>{(['SUGGEST_ONLY', 'APPROVAL_REQUIRED', 'ALLOWED'] as Permission[]).filter(option => permissionRank(option) <= permissionRank(capability.effectiveAutonomy)).map(option => <option key={option} value={option}>{permissionLabel(option)}</option>)}</select></div>)}</div></section><section className="border-t border-border pt-5"><h3 className="text-xs font-bold tracking-widest text-text-muted mb-3">WHEN TO INVOLVE ME / ESCALATION</h3><p className="text-sm text-text-secondary">{agent.charter.escalationRules.join(' ')}</p></section><section className="border-t border-border pt-5"><h3 className="text-xs font-bold tracking-widest text-text-muted mb-3">ACCESS</h3><div className="space-y-2">{Object.entries(agent.sourceAccess).map(([scope, level]) => <div key={scope} className="flex justify-between text-sm"><span className="text-text-secondary">{scope.replaceAll('_', ' ')}</span><span className="text-text-primary">{level.replaceAll('_', ' ')}</span></div>)}</div></section><section className="border-t border-border pt-5"><h3 className="text-xs font-bold tracking-widest text-text-muted mb-3">MODEL STRATEGY</h3><select disabled={saving} value={agent.operatorPolicy.modelStrategy} onChange={event => patch({ modelStrategy: event.target.value as any })} className="bg-surface border border-border text-text-primary text-sm rounded-lg px-3 py-2"><option value="AUTOMATIC">Automatic</option><option value="LOCAL_ONLY">Local only</option><option value="PRIVATE_ONLY">Private only</option><option value="HIGHEST_QUALITY">Highest quality</option></select><p className="text-xs text-text-muted mt-2">Strategy cannot bypass model acceptance gates.</p></section><section className="border-t border-border pt-5"><h3 className="text-xs font-bold tracking-widest text-text-muted mb-3">RUNTIME BOUNDARY</h3><p className="text-sm text-text-secondary">This agent can participate only in durable internal read/preparation work. It cannot execute tools, provider mutations, or outbound actions.</p></section><section className="border-t border-border pt-5"><h3 className="text-xs font-bold tracking-widest text-text-muted mb-3">ADVANCED READINESS</h3><div className="space-y-2">{agent.capabilities.map(capability => <div key={capability.capabilityId} className="flex items-start justify-between gap-4 text-sm"><span className="text-text-primary">{capability.capabilityId.replaceAll('_', ' ')}</span><span className={cn('text-right text-xs', agentStateClass(capability.availability))}>{capability.availability.replaceAll('_', ' ')}</span></div>)}</div></section></div>;
};
